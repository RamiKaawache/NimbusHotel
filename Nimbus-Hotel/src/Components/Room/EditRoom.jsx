...
import { getRoomById, updateRoom } from "../Utilities/APIfxns";
    ...
    const handleSubmit = async (e) => {
		...
		try {
			...
			if (response.status === 200) {
				setSuccessfulMessage("The Room has been edited successfully.")
				const updatedRoomData = await getRoomById(roomId)
				setRoom(updatedRoomData)
				setImagePreview(updatedRoomData.photo)
				setErrorMessage("")
			} else {
				setErrorMessage("There was an Error in editing the room.")
			}
		} catch (error) {
			console.error(error)
